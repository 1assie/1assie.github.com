...
jQuery(document).ready(function(){
    $('body').append('<div id="popup" style="position:absolute; display:none; z-index:200;"></div>');
    css_width = $('#popup').width();
    ...
    $(document).mousemove(function(e){ 
        ...
        x += 40; // important: if the popup is where the mouse is, the hoverOver/hoverOut events flicker
        y -= 40; 

        var x_y = nudge(x,y); // avoids edge overflow
      
        // remember: the popopup is still hidden
...
function popup(msg, width)
{
    if (ready) {
        ...
        $('#popup').html(msg).width(width).show();
        ...
        $(t).unbind('mouseout').bind('mouseout', 
            function(e){
                $('#popup').hide().width(default_width);
            }
        );
    }
}
...
function nudge(x,y)
{
    ...
    var xtreme = $(document).scrollLeft() + win.width() - $('#popup').width() - minMargin;
/*    if(x > xtreme) {
        x -= $('#popup').width() + 2 * minMargin;
    }
    x = max(x, 0);

    // When the mouse is too far down, move window up
    if((y + $('#popup').height()) > (win.height() +  $(document).scrollTop())) {
        y -= $('#popup').height() + minMargin;
    }
*/
    return [ x, y ];
}
